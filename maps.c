...
char *get_path(int pid)
{
    ...
    strcpy(ret, path);
    if (strstr(ret, ".so")) {
        fprintf(stderr, "Process ID: %d appears to be a shared library; file must be an "
                        "executable. (path: %s)\n",
                pid, ret);
        exit(-1);
    }
...
void get_address_space(struct address_space *addrspace, int pid, char *path)
{
    ...
    for (lc = 0, p = buf; fgets(buf, sizeof(buf), fd) != NULL; lc++) {
        ...
        if ((char *)strchr(buf, '/') && lc == 0) {
            ...
            addrspace[TEXT_SPACE].size =
                addrspace[TEXT_SPACE].evaddr - addrspace[TEXT_SPACE].svaddr;
        }
        ...
        if ((char *)strchr(buf, '/') && strstr(buf, path) && strstr(buf, "rw-p")) {
            ...
            addrspace[DATA_SPACE].size =
                addrspace[DATA_SPACE].evaddr - addrspace[DATA_SPACE].svaddr;
        }
        /*
     ...
     */
        if (strstr(buf, "[heap]")) {
            ...
            addrspace[HEAP_SPACE].size =
                addrspace[HEAP_SPACE].evaddr - addrspace[DATA_SPACE].svaddr;
        }
        /*
        ...
        if (strstr(buf, "[stack]")) {
            ...
            addrspace[STACK_SPACE].size =
                addrspace[STACK_SPACE].evaddr - addrspace[STACK_SPACE].svaddr;
        }
    }
}
