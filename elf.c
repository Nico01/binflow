...
char *get_section_by_range(handle_t *h, unsigned long vaddr)
{
    ...
    for (i = 0; i < h->shdr_count; i++) {
        if (vaddr >= h->sh_range[i].sh_addr &&
            vaddr <= h->sh_range[i].sh_addr + h->sh_range[i].sh_size)
...
void locate_dynamic_segment(handle_t *h) { h->elf.dyn = (ElfW(Dyn) *)&h->elf.mem[h->elf.dynOff]; }
