...
struct branch_instr branch_table[64] = {
    ...
    { "jmp", 0xeb }, { "jmp", 0xe9 }, { "jmpf", 0xea }, { NULL, 0 }
};
...
int build_code_profile(handle_t *h)
{
    ...
    for (j = 0; j < count; j++) {
        ...
        if ((branch_instr = search_branch_instr(insn[j].bytes[0])) != NULL) {
            ...
            h->branch_site[h->branch_count].branch.target_vaddr = target_address =
                strtoul(insn[j].op_str, NULL, 16);
        ...
        case 0xE8:
            ...
            h->branch_site[h->branch_count].branch.target_vaddr = target_address =
                strtoul(insn[j].op_str, NULL, 16);
            h->branch_site[h->branch_count].branch.target_offset =
                target_address - callsite - sizeof(uint32_t);
